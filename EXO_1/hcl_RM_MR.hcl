...
int srcA = [
    icode in { RRMOVL, OPL, PUSHL } || (icode == RMMOVL) && (ifun == 0) : rA;
...
bool mem_write =
    icode in { PUSHL, CALL } || (icode == RMMOVL && ifun == 0);

## Select memory address
int mem_addr = [
    icode in { PUSHL, CALL, RMMOVL} : valE;
...
int mem_data = [
    # Value from register
    icode in { PUSHL } || (icode == RMMOVL && ifun == 0): valA;
    # Return PC address
    icode == CALL : valP;
