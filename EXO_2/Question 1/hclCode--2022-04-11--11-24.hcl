...
bool need_regids =
    icode in { RRMOVL, OPL, IOPL, IRMOVL, RMMOVL } || ((icode == CALL) && (ifun == 1)) || ((icode == RET) && (ifun == 1));
...
bool instr_valid =
    icode in { NOP, HALT, RRMOVL, IRMOVL, RMMOVL,
               OPL, IOPL, JXX, CALL, RET} ;

## Decode stage ####################################################

## What register should be used as the A source?
int srcA = [
    icode in { RRMOVL, OPL} || ((icode == RMMOVL) && (ifun == 0))  || ((icode == CALL) && (ifun == 1)) : rA;
    icode in { RET } : RESP;
...
int dstE = [
    ...
    icode in { RET, CALL }: RESP;
    1 : RNONE;  # Don't need register for writing
];

## What register should be used as the M destination?
int dstM = [
    ((icode == RET) && (ifun == 1)) || ((icode == RMMOVL) && (ifun == 1)): rA;
    1 : RNONE;  # Don't need register for writing
];
...
int aluB = [
    icode in { RMMOVL, OPL, IOPL, CALL, RET } : valB;
...
bool mem_read =
    icode in { RET } || ((icode == RMMOVL) && (ifun == 1));

## Set write control signal
bool mem_write =
    icode in { CALL } || ((icode == RMMOVL) && (ifun == 0));

## Select memory address
int mem_addr = [
    icode in { CALL, RMMOVL} : valE;
    icode in { RET } : valA;
...
int mem_data = [
    # Value from register
    ((icode == CALL) && (ifun == 1)) || ((icode == RMMOVL) && (ifun == 0)): valA;
    # Return PC address
